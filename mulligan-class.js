...
const VERSION='mulligan-class-clean-1.5.1';WB.registerModule('mulligan-class',VERSION);
let manualKey='';
...
function mulliganStructure(src){const x=src.getContext('2d',{willReadFrequently:true}),x0=Math.floor(src.width*.15),x1=Math.ceil(src.width*.80),y0=Math.floor(src.height*.20),y1=Math.ceil(src.height*.90),im=x.getImageData(x0,y0,Math.max(1,x1-x0),Math.max(1,y1-y0)).data;let n=0,dark=0,sum=0;for(let i=0;i<im.length;i+=4){const v=Math.max(im[i],im[i+1],im[i+2])/255;n++;sum+=v;if(v<.28)dark++}const darkFrac=n?dark/n:0,meanV=n?sum/n:0;return{pass:darkFrac>=.20,darkFrac:+darkFrac.toFixed(3),meanV:+meanV.toFixed(3)}}
...
function mulliganPlan(firstTurn){const v=WB.video,early=firstTurn<6;let start,end,target,step;if(early){start=.25;end=Math.min(v.duration-.2,Math.max(start,firstTurn-.40));target=Math.max(start,Math.min(end,firstTurn-1));step=.25}else{start=Math.max(.5,firstTurn-8);end=Math.min(v.duration-.2,firstTurn-2);target=Math.max(start,Math.min(end,firstTurn-3));step=.5}const times=[];for(let t=start;t<=end+.001&&times.length<30;t+=step)times.push(+t.toFixed(2));return{firstTurn,start,end,target,step,mode:early?'short-start-adaptive':'standard',requireStructure:early,times}}
