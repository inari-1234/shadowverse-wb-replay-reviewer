(()=>{
  const PATCH='4.10.4-20260915-16a';
  ...
  function installFinalHeader(){
    ...
    st.textContent=`
      ...
      header>p:first-of-type::after{content:'Build 2026.09.15-16a / UI重複防止・解析速度維持';font-size:12px!important;color:#9ba8bf}
  ...
  function mountLatestBar(){
    ...
    if(!bar){
      ...
      bar.innerHTML='<button id="wbForceLatest4104" type="button" style="padding:7px 10px;background:#2563eb">最新版に更新</button><button id="wbRecoverView4104" type="button" style="padding:7px 10px">表示を復旧</button><span id="wbUpdateStatus4104" style="font-size:11px;color:#9ba8bf">v4.10.4 / Build 16a</span>';
      ...
      $q('#wbForceLatest4104')?.addEventListener('click',async()=>{
        ...
        try{
          if('serviceWorker'in navigator){const reg=await navigator.serviceWorker.register('./sw.js?v=4.10.4-20260915-16a',{updateViaCache:'none'});await reg.update()}
          if('caches'in window){const ks=await caches.keys();await Promise.all(ks.filter(k=>k.startsWith('wb-review-')&&k!=='wb-review-v4-10-4-20260915-16a').map(k=>caches.delete(k)))}
  ...
  safeLog('patch-v4104-active',{feature:'single-latest-bar-no-legacy-duplicates'});
})();