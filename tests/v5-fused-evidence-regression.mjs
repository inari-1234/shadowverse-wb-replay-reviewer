...
const evidence=JSON.parse(fs.readFileSync(new URL('./v5-real-video-validation-cases.json',import.meta.url),'utf8'));
assert.equal(evidence.policy.validation,false);
assert.equal(evidence.policy.snapshotOnly,true);
const floor=evidence.pipeline.visualFloor;
const negative=evidence.negatives.find(x=>x.id==='all-targets-absent-10s');
assert.ok(negative);

const positiveSources=Object.fromEntries(evidence.positives.map(x=>[x.cardId,x.medianScores]));
for(const cardId of Object.keys(positiveSources)){
  const pos=fuseVisualEvidenceSources(positiveSources[cardId],floor);
  const neg=fuseVisualEvidenceSources(negative.globalMaxScores[cardId],floor);
  assert.equal(pos.sourceConsensus,true,`${cardId} current-upload positive must have 2-of-3 source consensus`);
  assert.ok(pos.supportingCount>=2);
  assert.equal(neg.sourceConsensus,false,`${cardId} current-upload negative must not satisfy source consensus`);
...
const quickRows=['f1','f2','f3','f4'].map((frameKey,i)=>({
  videoKey:'dense-quick',visualFrameId:frameKey,slot:4,
...
const negativeRows=['n1','n2','n3','n4'].map((frameKey,i)=>({
  videoKey:'synthetic-negative',visualFrameId:frameKey,slot:1,
...
const tracking=evidence.historicalSnapshots.tracking;
const quickTrack=summarizeTrackEvidence(tracking.find(x=>x.cardId==='quickBlader'));
...
for(const row of tracking.filter(x=>x.cardId==='barbaros')){
...
console.log(JSON.stringify({
  ...
  historicalTrackingCases:tracking.length
},null,2));
