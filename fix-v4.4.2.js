(()=>{
  const PATCH='4.4.2-20260916-22';
  const $q=s=>document.querySelector(s);
  const header=$q('header h1'),sub=$q('header p');
  if(header) header.textContent='シャドバWB リプレイ診断 v4.4.2';
  if(sub) sub.textContent='Build 2026.09.16-22 / 初期マリガン区間を独立抽出・クラス判定はv4.6.8へ統合';
  ...
  panel.innerHTML=`
    <h2>3. マリガン・クラス確認</h2>
    <p id="previewStatus442" class="help">動画冒頭から最初の連続した初期4枚区間を探し、代表画像を1枚だけ表示します。</p>
  ...
  function plan(){const start=1,end=Math.min(9.5,video.duration-.2),times=[];for(let t=start;t<=end+.001&&times.length<30;t+=.5)times.push(+t.toFixed(2));return {first:null,start,end,times}}
  ...
  async function buildPreview(){if(running||!video?.src||!isFinite(video.duration)||video.duration<=0)return;running=true;const token=++runToken,original=video.currentTime;video.pause();if(still)still.style.display='none';if(stillTime)stillTime.textContent='';if(previewStatus)previewStatus.textContent='最初の4枚表示区間を探しています…';if(classStatus)classStatus.textContent='代表画像取得後に固定クラスアイコンを判定します…';try{const p=plan(),rows=[];for(const t of p.times){if(token!==runToken)throw new Error('cancelled');await seekSafe(t);const src=frameCanvas(1200),hs=handScore(src);rows.push({time:t,handCount:hs.count,handScore:hs.total});const ready=firstFourRun(rows);if(ready.length&&rows[rows.length-1].handCount!==4)break}const run=firstFourRun(rows);let chosen=run.length?run[run.length-1]:rows.slice().sort((a,b)=>b.handCount-a.handCount||b.handScore-a.handScore)[0];if(chosen){await seekSafe(chosen.time);const src=frameCanvas(1500),copy=copyFrame(src);if(still){still.src=copy.toDataURL('image/jpeg',.88);still.style.display='block'}if(stillTime)stillTime.textContent=`初期4枚：${fmt(chosen.time)} / 最初の4枚区間 ${run.length}フレーム`;window.mulliganPreview442={patch:PATCH,time:chosen.time,handCount:chosen.handCount,score:chosen.handScore,firstPP:null,scanStart:p.start,scanEnd:p.end,run:run.map(r=>r.time),sampleCount:rows.length,earlyStop:rows.length<p.times.length};window.classDetection442=null;if(classSelect)classSelect.value='';if(classStatus)classStatus.textContent='代表画像を取得しました。クラス判定はv4.6.8固定アイコン判定へ引き渡します。';try{log('class-detect-v442-skipped',{patch:PATCH,reason:'delegated-to-v468',time:chosen.time})}catch{}}if(previewStatus)previewStatus.textContent=chosen?.handCount===4?'最初の連続した4枚表示区間から1枚を選びました。':'4枚区間を確定できなかったため候補画像を表示しています。';try{log('mulligan-preview-v442',{...window.mulliganPreview442,samples:rows})}catch{}}catch(err){if(err?.message!=='cancelled'&&previewStatus)previewStatus.textContent='プレビュー作成失敗：'+(err?.message||String(err))}finally{try{if(token===runToken)await seekSafe(original)}catch{}running=false}}
  ...
  try{log('patch-v442-active',{patch:PATCH,feature:'early-mulligan-run-independent-of-pp-ocr-class-delegated-to-v468'})}catch{}
})();