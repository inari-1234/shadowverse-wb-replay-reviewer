(()=>{
'use strict';
const APP={version:'4.13.70',build:'4.13.70-20260926-clean-13-70',revision:'clean-13-70',subtitle:'Build 2026.09.26-clean-13-70 / 動画File受け渡し計測・Files優先導線'};
...
WB.updateTurnPick=()=>{const n=Number(WB.$('#turnPick')?.value)||1,row=WB.turnForTarget(n),go=WB.$('#goTurn'),cap=WB.$('#captureScene'),cmp=WB.$('#leCompare');if(go)go.disabled=!row||!!WB.task;if(cap)cap.disabled=!WB.videoMeta||!!WB.task;if(cmp)cmp.disabled=!WB.videoMeta||!WB.turnTimeline.length||!!WB.task;const time=WB.$('#turnTime'),st=WB.$('#turnStatus');if(time)time.value=row?WB.fmt(row.time):'未認識';if(st)st.textContent=row?`${n}Tを ${WB.fmt(row.time)} と認識しています。`:`${n}Tはまだ認識されていません。`};
