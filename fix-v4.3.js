(()=>{
  const PATCH='4.6.3-bootstrap-20260914-30';
  ...
  if(!mullPanel){
    ...
    mullPanel.innerHTML='<h2>3. マリガン自動抽出</h2><p id="mulliganStatus43" class="help">最新版を読み込み中…</p><div id="mulliganDetail43" class="ocrRead">v4.6.3 を準備しています。</div>';
    const anchor=prepPanel||$q('#scanTurns')?.closest('.panel');if(anchor)anchor.parentNode.insertBefore(mullPanel,anchor.nextSibling);
  }
  function load463(){
    if(window.__wb463Loaded)return;
    ...
    s.onload=()=>{window.__wb463Loaded=true;try{log('patch-v463-bootstrap-loaded',{patch:PATCH})}catch{}};
  ...
  load431();try{log('patch-v463-root-bootstrap',{patch:PATCH})}catch{}
})();