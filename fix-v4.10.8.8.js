(()=>{
  const PATCH='4.10.8.8-20260917-20s';
  const BUILD_LABEL='Build 2026.09.17-20s / 起動競合源を停止';
  ...
  function stabilize(reason='manual'){
    ...
    if(st&&!/(準備中|更新中|確認中|再起動|復旧|失敗|取得中|反映中)/.test(String(st.textContent||''))&&st.textContent!=='v4.10.8 / Build 20s / 起動競合停止'){st.textContent='v4.10.8 / Build 20s / 起動競合停止'}
  ...
  async function migrateCanonicalWorker(){
    if(!('serviceWorker'in navigator)||sessionStorage.getItem('wbCanonicalSw20s')==='1')return;
    try{
      const reg=await navigator.serviceWorker.getRegistration();const src=reg?.active?.scriptURL||'';
      if(!src)return;const u=new URL(src);
      if(!u.search)return;
      sessionStorage.setItem('wbCanonicalSw20s','1');
  ...
  function verify(reason='verify',emit=true){
    ...
    const state={patch:PATCH,reason,header:h?.textContent||'',subheader:p?.textContent||'',canonicalStyle:!!s&&!s.disabled,legacyEnabled:LEGACY_STYLE_IDS.filter(id=>{const x=$q('#'+id);return x&&!x.disabled}),legacyUiFrozen:!!window.__wbLegacyUiFrozen,controllerScript:navigator.serviceWorker?.controller?.scriptURL||null,repairs,checkedAt:new Date().toISOString()};
    state.ok=state.header==='シャドバWB リプレイ診断 v4.10.8'&&state.subheader===BUILD_LABEL&&state.canonicalStyle&&state.legacyEnabled.length===0&&state.legacyUiFrozen;
  ...
  safeLog('patch-v41088-active',{feature:'canonical-header-stable-sw-legacy-ui-freeze'});
})();
